...
#define MAX_SENSOR_DISTANCE 100 // was 500. Maximum sensor distance can be as high as 500cm, no reason to wait for ping longer than sound takes to travel this distance and back.
...
#define NO_ECHO 6500	//was 0  	// Value returned if there's no ping echo within the specified MAX_SENSOR_DISTANCE or max_cm_distance.
...
class NewPing {
	public:
		NewPing(uint8_t trigger_pin /*= 3*/, uint8_t echo_pin /*= 4*/, int max_cm_distance = MAX_SENSOR_DISTANCE);
		NewPing() {}
		unsigned int ping();
		...
		void init(uint8_t trigger_pin, uint8_t echo_pin, int max_cm_distance = MAX_SENSOR_DISTANCE);
